...
- AI 改圖分頁的版面與提示詞工具全面對齊生成分頁：左側設定面板從 4 段編號卡片改成生成分頁同構的 `composer` +「進階設定」摺疊區（首屏＝模式／參考圖／指令／風格＋幅度／主按鈕，其餘收起），工作區也改成左設定右預覽的雙欄（980px 以下回單欄）。改圖指令新增「幫我補完整」「轉成英文指令」「加效果」三個工具——`prompt-transform.js` 改成 ctx 參數化，兩個分頁共用同一條管線而非各寫一份 fetch；加效果重用既有的 `PromptEnhancer`。新增風格選單，選定的風格會以中文子句併進送出的指令（`auto` 不加）。
- 修正改圖分頁在公開站無法通過人機驗證：`turnstileGate` 原本位於生成分頁的 DOM 內，切到改圖分頁時整個 tabpanel 隱藏，使用者看不到驗證框卻仍會送出，`TURNSTILE_REQUIRED=true` 時必定 403。改為放在分頁外層由兩個分頁共用，並在改圖送出前先自行檢查 token。
- 改圖的「修改幅度」改成分段滑桿：底層是原生 `input[type=range]`，拖曳／點擊／方向鍵／觸控都由瀏覽器提供，`aria-valuetext` 同步報出微調／適中／大改。填充延伸到 thumb 右緣而非中心，thumb 尺寸集中在 `--seg-thumb` 一個變數，填充長度由 CSS 依 `--seg-index` 推算（JS 只寫索引）。樣式 class `.segmented-slider` 可重用於其他有序的程度選擇。
- 生成分頁的「進階設定」內部分成「提示詞／生成品質／輸出」三組並加分隔線：原本單一摺疊區混了改提示詞、調生成行為、定輸出規格三種性質的控制項，一展開全部湧出。控制項沒有增減，只是變得可掃；順帶說明尺寸會由「用途」自動帶入（此前沒寫，容易誤以為與首屏的用途重複）。
- 啟用生成後視覺 QA，並改用 NVIDIA VLM：`VISION_QA_PROVIDER` 新增於兩個 runtime，預設 `nvidia`——與生圖共用同一把 `NVIDIA_API_KEY`，不另外吃付費配額；指定的後端缺金鑰時自動退到另一邊，兩邊都沒有才停用。`/api/health` 新增 `visionQaProvider` 供診斷。此功能先前一直帶著 `VISION_QA_ENABLED=false` 出貨，因此下列兩個問題從未在真實圖片上暴露過：其一，`maxOutputTokens` 在 thinking 模型上是 thinking 與輸出共用的預算，700 會被 thinking 吃掉（實測複雜圖片用掉 669）導致 JSON 從中間截斷，現改為關閉 thinking 並提高上限；其二，`VISION_QA_TIMEOUT_MS` 的 5000 對 1024×1024 圖片遠遠不夠，現為 60000（NVIDIA 實測中位 8.4 秒、最慢 17.0 秒，但尾端曾出現 38 秒）。注意該設定只有 Worker 版會讀，且受 `vision.js` 的上限夾住；FastAPI 版走 `PROMPT_LLM_TIMEOUT_SECONDS`。
- service worker 快取版本加上防呆閘門：`ASSET_FINGERPRINT` 記錄 `app/static` 下所有 HTML/CSS/JS 的雜湊，測試比對不符就失敗並直接給出應填的雜湊與該 bump 的 `CACHE_NAME`。動機是實際踩過——改了前端卻沒 bump 快取版本時，network-first 讓舊快取只是靜靜留著當 fallback，不會壞掉但也不會觸發重抓、前端的「新版本已準備好」提示不跳，回訪的分頁繼續渲染舊介面。雜湊前先把行尾正規化成 LF，避免 Windows 與 CI 算出不同值。
- 修正 `test_deployment_preflight` 與 `test_smoke_live_provider` 共 11 個測試在 Windows 上必定失敗：`subprocess.run` 用了 `text=True` 卻沒指定 `encoding`，Windows 預設以 cp950 解碼腳本的中文輸出，reader thread 拋 `UnicodeDecodeError`、`stdout`/`stderr` 變成 `None`，所有解析這兩個串流的測試一起掛掉，連帶讓 `npm run deploy` 卡在 offline verify 閘門。改為固定 `encoding="utf-8"`。
- AI 改圖分頁補齊到與生成分頁同級的工具箱：新增「快速指令庫」（依一般／角色／產品三種模式各給一組中文常用指令 chip，點一下帶入可再編輯、不消耗額度，對應生成分頁的「幫我想梗」）、「修改幅度」三檔（微調／適中／大改）與「保留項目」複選（臉部／文字 Logo／構圖／配色）——後端 `/edit` 只吃 prompt + images，這兩項是靠 `composeEditPrompt()` 組成中文子句表達，不動後端契約。結果區新增「前後對比」滑桿（結果圖以 `clip-path` 依 `--split` 裁切露出下層原圖，純 CSS 無新相依）、「用結果再改一次」（結果 dataURL → Blob → 走同一條縮圖路徑塞回參考圖，形成迭代閉環）與「複製指令」（複製實際送出的完整 prompt）。改圖結果現在會 dispatch 既有的 `imagegen:generated` 事件寫進歷史記錄牆（帶 `AI 改圖` 標籤，可下載／收藏／標籤／分享），失敗時渲染與生成分頁同一份 `FailureAdvice` 建議，並沿用 `.canvas-statusbar` 顯示狀態／模式／參考圖數／幅度／provider 來源。改圖記錄在歷史牆的「畫質」欄會顯示「平衡」（`qualityLabel` 只看 steps/cfg_scale，改圖沒有這兩個參數），靠 `AI 改圖` 標籤區分來源。
- 生成新增 NVIDIA 熔斷器(circuit breaker):NVIDIA 因基礎設施失敗(5xx/逾時)暗掉時,標記為 down 一段冷卻期(180 秒),冷卻期內的請求直接跳過 NVIDIA、秒切後備鏈,省掉每張圖 ~60 秒的逾時等待;冷卻期過後自動半開,下一個請求重探 NVIDIA,恢復就自動切回。Worker 用 Cache API(免綁定、每資料中心各自學、依 max-age 自動過期)、FastAPI 用行程記憶體。只有 infra 失敗會觸發熔斷,內容過濾(422)/限流(429)不會。NVIDIA 正常時維持完整逾時,不影響「精緻」等本來就較慢的合法生成。
...
- 歷史單筆刪除加確認：先前批次刪除／清空都有確認、最易誤觸的單筆刪除卻沒有，現在補上「無法復原」確認（雲端作品不受影響）。
- 修正頂欄快捷鍵提示「1–4 切換工具」為「1–3」（實際只有生成／改圖／歷史三個分頁）。
- 移除頂欄裝飾性假麵包屑「創作工作區／新作品」：暗示不存在的專案管理功能，且作品集子系統已移除。
- 教學「不要再顯示」預設勾選：按「×」或點背景關閉教學後不再每次重彈；想再看可取消勾選或按頂欄「?」。
- AI 改圖參考圖標籤中文化並改 1 起算：縮圖標籤、alt 與角色下拉的 aria-label 由「image 0」改為「第 1 張」，與畫面順序一致（送給模型的多圖索引指令維持 image 0/1 不變）。
- 逾時失敗建議改為可執行：原本建議「暫時改用高品質模型」，但畫質選單已合併隱藏、且一律走高品質，該建議無法照做；改為「換一個畫面編號、縮短描述，或稍後再試」。
- 教學步驟「選畫質與尺寸」改為明確引導：說明畫質與尺寸在下方「進階設定」摺疊區內，不調也有合理預設可直接生成（此前照步驟找不到入口）。
- 修正歷史畫質顯示與篩選失效：先前歷史記錄未持久化 steps／cfg_scale，導致「草稿／平衡／精緻」永遠顯示「平衡」、依畫質篩選只會落在平衡。生成記錄（單張與批次）現在帶入 steps／cfgScale，history-store 正規化亦保留這兩欄，畫質標籤與篩選才真正生效。
