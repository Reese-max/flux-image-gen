...
| 狀態 | 項目 | 驗收方式 | 對應任務 | Release blocker |
|---|---|---|---|---|
| [ ] | AgentStep 流程 | 智慧體模式顯示解析、補全、選模型尺寸、生成、QA、推薦、修改建議；失敗會標示卡在哪一步 | TASK-015, TASK-016 | 是 |
| [ ] | 多張生成與最佳圖推薦 | 預設 4 張；每張有 metadata / QAReport；系統推薦一張且可選其他張 | TASK-020, TASK-021, TASK-022 | 是 |
| [ ] | Vision QA 部署抽驗 | 若對外宣稱可檢查手指、臉部、文字亂碼，需設定 `VISION_QA_ENABLED=true` + `GEMINI_API_KEY` 並用真實圖抽驗 | TASK-021, TASK-023 | 是 |
| [ ] | 自動重試成本上限 | 製造嚴重模糊 / 主體缺失測試，確認最多重試一次且記錄原因 | TASK-023 | 是 |
| [ ] | 下一步建議 | 成功後至少 3 個一鍵可套用建議，套用後不自動消耗額度 | TASK-024 | 否 |
...
| [ ] | 未完成編輯 stub | 局部修改、換背景、擴圖、去背、加文字、調整比例皆顯示「即將推出」，不可誤導可用 | TASK-035 | 是 |
